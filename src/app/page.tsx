...
const KeywordTags = ({ keywords, onKeywordSelect }: { keywords: Keyword[], onKeywordSelect: (keyword: string) => void }) => {
  return (
  ...
  return (
    <div className="space-y-6">
      <div className="space-y-4 p-4 rounded-lg bg-card border">
        ...
        <KeywordTags keywords={keywords} onKeywordSelect={handleKeywordSelect} />
      </div>

      <div className="space-y-4">
        {loading && Array.from({ length: 3 }).map((_, index) => (
          <div key={index} className="p-4 rounded-lg bg-card border space-y-3">
        ...
        {!loading && filteredQuestions.length === 0 && (
          <div className="text-center py-12 text-muted-foreground">
            ...
            <p className="text-sm mt-1">Try a different search term or ask a new question!</p>
          </div>
        )}
      </div>
    </div>
