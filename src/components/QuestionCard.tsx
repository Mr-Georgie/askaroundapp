  ...
  return (
    <Card className="overflow-hidden transition-all duration-300 ease-in-out hover:shadow-xl hover:scale-[1.02]">
          ...
          <div className="flex-1">
            ...
            <div className="flex items-center gap-2 text-sm text-muted-foreground mt-1">
              <Avatar className="h-5 w-5">
                <AvatarImage src={question.user.avatarUrl} alt={question.user.name} />
      ...
      <CardFooter className="bg-muted/50 px-6 py-3 flex items-center justify-between">
        <VoteButtons collection="questions" docId={question.id} initialVotes={question.votes} />
        <div className="flex items-center gap-4 text-sm text-muted-foreground">
          <Link href={`/question/${question.id}#answers`} className="flex items-center gap-1.5 hover:text-primary transition-colors">
