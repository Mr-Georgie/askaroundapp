...
import type { Question, Answer, User, SageMessage, SageConversation, AppNotification, Keyword, AdminStats, AdminSearchResult } from './types';
...
export const addAnswer = async (questionId: string, answerData: { text: string; user: User; photoDataUrl?: string }) => {
  try {
    ...
    const newAnswerPayload: { [key: string]: any } = {
    ...
    if (answerData.photoDataUrl) {
      newAnswerPayload.photoDataUrl = answerData.photoDataUrl;
    }

    batch.set(newAnswerRef, newAnswerPayload);
...
export const searchContentForAdmin = async (searchText: string): Promise<AdminSearchResult[]> => {
    ...
    questionsSnapshot.forEach(doc => {
        ...
        if (data.text.toLowerCase().includes(textLower)) {
            results.push({ ...convertTimestamps(data), id: doc.id, type: 'question' });
        }
    });
